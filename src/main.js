...
import { enhanceUI } from './ux.js';
const root=document.querySelector('#app');let state=createInitialState(20260819),selected={type:'stronghold',id:'kibi_camp'},selectedTactical=null,last=performance.now(),worldAcc=0,tacticalAcc=0;const saveKey='wano-sandbox-save-v2';const getState=()=>state,setSelected=v=>selected=v,getSelectedTactical=()=>selectedTactical,setSelectedTactical=v=>selectedTactical=v;
function draw(){root.innerHTML=render(state,selected,selectedTactical);bind(root,getState,setSelected,getSelectedTactical,setSelectedTactical,draw,save,load);enhanceUI(root,getState,selected,setSelected,draw)}
function save(){localStorage.setItem(saveKey,serialize(state));event(state,'World state saved to this browser.','system');draw()}
function load(){try{const raw=localStorage.getItem(saveKey);if(!raw)throw Error('No compatible local save found.');state=deserialize(raw);selected={type:'stronghold',id:Object.values(state.strongholds).find(h=>h.owner===state.playerFaction)?.id||'kibi_camp'};selectedTactical=null;event(state,'World state loaded.','system');draw()}catch(err){event(state,err.message,'warning');draw()}}
