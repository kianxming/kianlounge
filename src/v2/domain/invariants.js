...
const activeArmyStatuses=new Set(['moving','waiting','battle','retreating','routed','stranded','destroyed']);
...
export function validateStrategyState(state,{throwOnError=false}={}){
  ...
  for(const [id,army] of Object.entries(state.armies||{})){
    ...
    if(army.status!=='destroyed'&&army.currentNodeId&&army.currentEdgeId){
      fail(errors,`army ${id} cannot occupy node ${army.currentNodeId} and edge ${army.currentEdgeId} simultaneously`);
    }
    ...
    if(army.operationId){
      ...
      if(!op)fail(errors,`army ${id} references missing operation ${army.operationId}`);
        ...
        if(op.armyId&&op.armyId!==id)fail(errors,`army ${id} operation ${op.id} belongs to ${op.armyId}`);
        if(terminalOperation.has(op.status)&&army.status!=='stranded')fail(errors,`army ${id} retains terminal operation ${op.id}`);
      }
    }
    ...
    }else if(army.battleId){
      fail(errors,`non-battle army ${id} still references battle ${army.battleId}`);
    }
  }
  ...
  for(const [id,op] of Object.entries(state.operations||{})){
    ...
    if(op.armyId){
      ...
      if(!terminalOperation.has(op.status)&&army&&army.operationId!==id&&army.status!=='battle'){
        fail(errors,`active operation ${id} is not owned by army ${op.armyId}`);
      }
    }
