...
function restoreScroll(s){const apply=()=>{const map=$('#map-scroll'),left=$('.side-panel.left'),right=$('.side-panel.right');if(map){map.scrollLeft=s.mx;map.scrollTop=s.my}if(left)left.scrollTop=s.ly;if(right)right.scrollTop=s.ry};apply();requestAnimationFrame(apply)}
function renderTop(){
...
function battleSideSummary(ids=[]){const armies=ids.map(id=>state.armies[id]).filter(Boolean),troops=armies.reduce((n,a)=>n+(a.troops||0),0),morale=armies.length?Math.round(armies.reduce((n,a)=>n+(a.morale??100),0)/armies.length):0,commander=state.officers[armies[0]?.commanderId];return{armies,troops,morale,name:commander?.name||factionName(armies[0]?.factionId)}}
...
async function runExecution(){
  ...
  const tick=()=>{if(state.phase!=='execution'){executionTimer=null;render();return}if(paused){executionTimer=setTimeout(tick,220);return}try{advanceOneDay(state,{reactiveAI:s=>runDailyReactiveDirector(s,{playerFactionId:s.playerFactionId})});const important=processNewEvents();if(!mapInteracting)render();if(important&&!mapInteracting)requestAnimationFrame(()=>showBanner(important))}catch(err){console.error(err);toast(`실행 오류: ${err.message}`);paused=true;return}const delay=speed===4?90:speed===2?180:360;executionTimer=setTimeout(tick,delay)};tick()
}
