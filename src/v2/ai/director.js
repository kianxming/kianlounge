...
function demobilizeBroken(state,fid){let n=0;for(const army of exhaustedArmies(state,fid)){if(commandBudget(state,fid).remaining<=0)break;if(disbandArmy(state,{factionId:fid,armyId:army.id}))n++}return n}
function replenishFrontArmies(state,fid,theater){let n=0;for(const nodeId of theater.frontlineNodeIds)for(const army of waitingAt(state,fid,nodeId)){if(commandBudget(state,fid).remaining<=0)return n;if((army.troops||0)<1200||(army.supplies||0)<70){if(reinforceArmy(state,{factionId:fid,armyId:army.id,troops:400,supplyPoints:50}))n++}}return n}
function formForces(state,fid,theater,profile){
  ...
  for(const s of candidates){if(existing+formed>=desired||commandBudget(state,fid).remaining<=0)break;const officer=bestOfficerAt(state,fid,s.nodeId,'command');if(!officer||s.troops<1900||s.food<1100)continue;const troops=Math.min(2400,Math.max(1000,Math.floor((s.troops-700)*.62/100)*100)),army=formArmy(state,{factionId:fid,nodeId:s.nodeId,commanderId:officer.id,troops,supplyPoints:Math.min(220,100+Math.floor(profile.logistics*100))});if(army)formed++}return formed;
}
function feedFront(state,fid,theater){if(commandBudget(state,fid).remaining<=0)return 0;const target=theater.frontlineNodeIds.map(id=>state.settlements[id]).filter(s=>s&&s.food<2600).sort((a,b)=>a.food-b.food)[0];if(!target)return 0;const donor=settlements(state,fid).filter(s=>s.nodeId!==target.nodeId&&s.food>5200).sort((a,b)=>b.food-a.food)[0];if(!donor)return 0;const officer=bestOfficerAt(state,fid,donor.nodeId,'logistics');if(!officer)return 0;return orderTransport(state,{factionId:fid,originNodeId:donor.nodeId,destinationNodeId:target.nodeId,commanderId:officer.id,cargo:{food:Math.min(1800,donor.food-3600)}})?1:0}
...
export function runFactionMonthlyDirector(state,factionId){
  ...
  summary.demobilized=demobilizeBroken(state,factionId);summary.replenished=replenishFrontArmies(state,factionId,theater);summary.formed=formForces(state,factionId,theater,profile);summary.transports=feedFront(state,factionId,theater);summary.offensives=planFactionMonth(state,factionId,{maxOrders:Math.max(1,Math.floor(1+profile.aggression*2))}).length;summary.domestic=domesticOrders(state,factionId,theater,profile);
  if(state.stats)state.stats.aiOrders=(state.stats.aiOrders||0)+summary.demobilized+summary.replenished+summary.formed+summary.transports+summary.offensives+summary.domestic;return summary;
}
...
function autoDetectThreats(state){
  for(const op of Object.values(state.operations||{})){
    if(op.type!=='army_march'||op.objective!=='attack'||op.status!=='marching')continue;const defender=state.graph.nodes[op.destinationNodeId]?.ownerFactionId;if(!defender||defender===op.factionId||state.intelligence.detectedByFaction?.[defender]?.[op.id])continue;
    const observation=observationDetectionBonus(state,defender,op.destinationNodeId),scouted=state.intelligence.scoutedNodes?.[defender]?.[op.destinationNodeId],scoutActive=Boolean(scouted&&scouted.untilDay>=state.day),horizon=7+Math.floor(observation/2)+(scoutActive?12:0);
    if((op.travelDaysRemaining??999)<=horizon)markOperationDetected(state,defender,op.id,{confidence:Math.min(1,.62+observation*.025+(scoutActive?.18:0))});
  }
}
